...
output "pas_buildpacks_backup_bucket" {
  value = length(aws_s3_bucket.buildpacks_backup_bucket) > 0 ? aws_s3_bucket.buildpacks_backup_bucket.0.bucket : ""
}

output "pas_droplets_backup_bucket" {
  value = length(aws_s3_bucket.droplets_backup_bucket) > 0 ? aws_s3_bucket.droplets_backup_bucket.0.bucket : ""
}

output "pas_packages_backup_bucket" {
  value = length(aws_s3_bucket.packages_backup_bucket) > 0 ? aws_s3_bucket.packages_backup_bucket.0.bucket : ""
}

output "pas_resources_backup_bucket" {
  value = length(aws_s3_bucket.resources_backup_bucket) > 0 ? aws_s3_bucket.resources_backup_bucket.0.bucket : ""
}
...
output "isoseg_target_groups" {
  value = [ compact(
    length(aws_lb_target_group.isoseg_80) > 0 aws_lb_target_group.isoseg_80.0.name : "",
    length(aws_lb_target_group.isoseg_443) aws_lb_target_group.isoseg_443.0.name : "",
    length(aws_lb_target_group.isoseg_4443) aws_lb_target_group.isoseg_4443.0.name : "")
  ]
}
